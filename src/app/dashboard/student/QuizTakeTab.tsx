...
const QuizTakeTab: React.FC<QuizTakeTabProps> = ({ quizId, onComplete, onBack, onStart }) => {
  ...
  const loadQuizData = async () => {
    try {
      ...
      const quizResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/${quizId}`);
      if (!quizResponse.ok) {
      ...
      const questionsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/${quizId}/questions`);
  ...
  const checkQuizAttempts = async (quizId: number) => {
    try {
      ...
      // Check if student has already attempted this quiz
      const attemptsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts?quiz_id=${quizId}&student_id=${studentId}`);
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      const submitResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts`, {
        method: 'POST',
        headers: {
