...
const QuizResultsTab: React.FC = () => {
  ...
  const loadQuizResults = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts?student_id=${studentId}`);
      ...
      const resultsWithDetails = await Promise.all(
        attempts.map(async (attempt: any) => {
          try {
            ...
            const quizResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/${attempt.quiz_id}`);
            ...
            const questionsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/${attempt.quiz_id}/questions`);
            let questions = [];
            ...
            const answersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts/${attempt.id}/answers`);
            let studentAnswers = {};
