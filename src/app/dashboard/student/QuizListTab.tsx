  ...
  useEffect(() => {
    const loadQuizzes = async () => {
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/accessible/${studentId}`;
        ...
        if (studentId) {
          ...
          for (let i = 0; i < apiQuizzes.length; i++) {
            ...
            try {
              const attemptsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts?quiz_id=${quiz.id}&student_id=${studentId}`);
