...
const UsersManagementTab: React.FC<UsersManagementTabProps> = ({
  ...
  const fetchParentDataForStudent = async (studentId: number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/students/${studentId}/parent`, {
        headers: {
  ...
  const fetchChildDataForParent = async (parentId: number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/parents/${parentId}/child`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken') || localStorage.getItem('token')}`,
        },
      });
  ...
  const handleEditUser = async (updatedUser: Student | Parent) => {
    try {
      ...
      const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001';
  ...
  const handleAddUser = async (newUser: Partial<Student | Parent>) => {
    ...
    if (activeTab === 'students') {
        ...
        const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001';
