        ...
        const children: Child[] = profile.children && profile.children.length > 0 
          ? await Promise.all(
            ...
            try {
              ...
              const backendUrl = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001';
  ...
  useEffect(() => {
    const loadChildProgressData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://51.77.195.224:3001'}/quizzes/attempts?student_id=${currentChild.id}`);
